...
int main()
{
	...
	struct DataTable* samples = dt_table_sample_rows(table, 10, true);

	if (samples->n_rows != 10)
	{
		fprintf(stderr, "Expected samples table to have 10 records but has %zu.\n", samples->n_rows);
		goto cleanup;
	}
